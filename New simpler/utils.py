...
# ------------------ Background solve slot ------------------
# The solve runs in a background thread, separate from the Streamlit UI thread,
# and its status/result are written to disk (not just st.session_state). This means:
...
# There's just one solve "slot" at a time (simplest case for a single-user tool) -
# starting a new solve overwrites the previous status/result.
...
def start_solve_job(job_fn, *args, **kwargs):
    """
    Runs job_fn(*args, **kwargs) in a background thread and returns immediately
    (non-blocking). job_fn must NOT call any Streamlit (st.*) commands - it runs
    outside any Streamlit script context.

    Status/result are persisted to disk so the UI can check on them later,
    independent of any particular browser session.
    """
    with _solve_lock:
